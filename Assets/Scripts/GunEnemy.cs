...
public class GunEnemy : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (!aiming)
        {
            if (safe)
            {
                if (transform.position.x > player.transform.position.x && rb.velocity.x > 2 && changeDir)
                {
                    changeDir = false;
                    float rnd = Random.Range(changeDirTime, changeDirTime + 0.8f);
                    Invoke("ChangeDir", rnd);
                }
                else if (transform.position.x < player.transform.position.x && rb.velocity.x < -2 && changeDir)
                {
                    changeDir = false;
                    float rnd = Random.Range(changeDirTime, changeDirTime + 0.8f);
                    Invoke("ChangeDir", rnd);
        ...
        else
        {
            ...
            {
                jump = true;
                float rnd = Random.Range(3.5f, 0.2f);
            ...
            velX = accel;
            rb.velocity += new Vector2(velX, velY) * Time.deltaTime;
            rb.velocity = new Vector2(Mathf.Clamp(rb.velocity.x, -maxSpeed * 0.75f, maxSpeed * 0.75f), rb.velocity.y);
        }
    ...
    public void TakeDamage(int damage)
    {
        ...
        if (lives <= 0)
        {
            ...
            Destroy(GetComponentInChildren<EnGunTrans>().gameObject);

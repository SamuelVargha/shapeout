...
public class MGunEnemy : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        {
            jump = true;
            float rnd = Random.Range(1.8f, 0.2f);
        ...
        if (!aiming)
        {
            if (safe)
            {
                if (transform.position.x > player.transform.position.x && rb.velocity.x > 0.3f && changeDir)
                {
                    changeDir = false;
                    float rnd = Random.Range(changeDirTime, changeDirTime + 0.8f);
                    Invoke("ChangeDir", rnd);
                }
                else if (transform.position.x < player.transform.position.x && rb.velocity.x < -0.3f && changeDir)
                {
                    changeDir = false;
                    float rnd = Random.Range(changeDirTime, changeDirTime + 0.8f);
                    Invoke("ChangeDir", rnd);
        ...
        {
            float rnd = Random.Range(4.2f, 0.2f);
            float rnd2 = Random.Range(0.6f, -1f);
            if (!jump && grounded)
            {
                jump = true;
                
                Invoke("AimJumpTimer", rnd);
            }
            ...
            velX = accel;
            rb.velocity += new Vector2(velX, velY) * Time.deltaTime;
            rb.velocity = new Vector2(Mathf.Clamp(rb.velocity.x, -maxSpeed * 0.35f, maxSpeed * 0.35f), rb.velocity.y);
        }
    ...
    private void AimJumpTimer()
    {
        ...
        if (safe)
        {
            Jump(jumpVel - (Random.Range(0.2f, 6f)));
        }
    }
    ...
    public void TakeDamage(int damage)
    {
        ...
        if (lives <= 0)
        {
            ...
            Destroy(GetComponentInChildren<EnMGunTrans>().gameObject);
